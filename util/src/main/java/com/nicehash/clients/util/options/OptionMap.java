...
public final class OptionMap implements Iterable<Option<?>>, Serializable {
  ...
  private static final ErrorMessages msg = ErrorMessages.INSTANCE;
  ...
  public <T> T get(Option<T> option) {
    Object tmp = this.value.get(option);
    if (tmp == null && option.isRequired()) {
      throw new IllegalArgumentException("No such option: " + option.getName());
    }
    return option.cast(tmp);
  }

  /**
   * Get the value of an option from this option map, with a specified default if the value is
   * missing.
   *
   * @param option the option to get
  ...
  /**
   * Create a two-valued option map. If both options are the same key, then only the second one is
   * added to the map.
   *
   ...
   * @since 3.0
   */
  public static <T1, T2> OptionMap create(
      Option<T1> option1, T1 value1, Option<T2> option2, T2 value2) {
    if (option1 == null) {
    ...
    final IdentityHashMap<Option<?>, Object> map = new IdentityHashMap<>(2);
  ...
  /**
   * A builder for immutable option maps. Create an instance with the {@link OptionMap#builder()}
   * method.
   */
  public static final class Builder {
    ...
    /**
     * Add all options from a properties file. Finds all entries which start with a given prefix
     * followed by '.'; the remainder of the property key (after the prefix) is the option name, and
     * the value is the option value.
     *
     * <p>If the prefix does not end with '.' character, a '.' will be appended to it before
     * parsing.
     *
      ...
      for (String name : props.stringPropertyNames()) {
        if (name.startsWith(prefix)) {
          ...
          try {
            ...
            msg.invalidOptionInProperty(optionName, name, e);
          }
        }
      }
      return this;
    ...
    public Builder set(Option<Integer> key, int value) {
      ...
      list.add(new OVPair<Integer>(key, value));
      return this;
    }
    ...
    public Builder set(Option<Long> key, long value) {
      ...
      list.add(new OVPair<Long>(key, value));
      return this;
    }
    ...
    /**
     * Add all the entries of a map. Any keys of the map which are not valid {@link Option}s, or
     * whose values are not valid arguments for the given {@code Option}, will cause an exception to
     * be thrown. Any keys which occur more than once in this builder will be overwritten with the
    ...
    /**
     * Add all entries from an existing option map to the one being built. Any keys which occur more
     * than once in this builder will be overwritten with the last occurring value.
     *
