...
public class OrderBookSync implements AutoCloseable {
    ...
    public OrderBookSync(SyncHandle handle, Map<BigDecimal, BigDecimal> bids, Map<BigDecimal, BigDecimal> asks) {
    ...
    /**
     * Be aware that this is not atomic operation with regard to getAsks()!!
     * As such, bids and asks may come from different updates.
     * Use snapshot() for atomic operation.
     */
    ...
    /**
     * Be aware that this is not atomic operation with regard to getBids()!!
     * As such, asks and bids may come from different updates.
     * Use snapshot() for atomic operation.
     */
    ...
    public OrderBook snapshot() {
        ...
        handle.readLock().lock();
        try {
            List<OrderBookEntry> bidsList = bids.entrySet()
                                                .stream()
                                                .map(e -> new OrderBookEntry(e.getKey(), e.getValue()))
                                                .collect(Collectors.toList());
            ob.setBids(bidsList);

            List<OrderBookEntry> asksList = asks.entrySet()
                                                .stream()
                                                .map(e -> new OrderBookEntry(e.getKey(), e.getValue()))
                                                .collect(Collectors.toList());
            ob.setAsks(asksList);
