...
/**
 * Custom serializer for a User Data stream event, since the API can return two different responses
 * in this stream.
 *
...
public class UserDataUpdateEventSerializer extends JsonSerializer<UserDataUpdateEvent> {
  ...
  public void serialize(
      UserDataUpdateEvent event, JsonGenerator gen, SerializerProvider serializers)
      throws IOException {
    ...
    if (accountUpdateEvent != null) {
      accountUpdateEvent.setEventType(
          UserDataUpdateEvent.UserDataUpdateEventType.ACCOUNT_UPDATE.getEventTypeId());
      accountUpdateEvent.setEventTime(event.getEventTime());
      gen.writeObject(accountUpdateEvent);
    } else {
      orderTradeUpdateEvent.setEventType(
          UserDataUpdateEvent.UserDataUpdateEventType.ORDER_TRADE_UPDATE.getEventTypeId());
