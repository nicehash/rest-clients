...
public class ExchangeClientImpl implements ExchangeClient, ServiceHandle<ExchangeService> {
  ...
  public List<TradeHistoryItem> getHistoricalTrades(String symbol, Integer limit, Long fromId) {
    return executeSync(getServiceInterface(), service.getHistoricalTrades(symbol, limit, fromId))
        .getList();
  }
  ...
  public List<AggTrade> getAggTrades(
    ...
    return executeSync(
            getServiceInterface(), service.getAggTrades(symbol, fromId, limit, startTime, endTime))
        .getList();
  }
  ...
  public TickerStatistic get24HrPriceStatistics(String symbol) {
    return executeSync(
            getServiceInterface(), service.get24HrPriceStatistics(Objects.requireNonNull(symbol)))
        .getSingle();
  }
  ...
  public TickerPrice getPrice(String symbol) {
    return executeSync(
            getServiceInterface(), service.getLatestPrice(Objects.requireNonNull(symbol)))
        .getSingle();
  }
  ...
  public BookTicker getBookTicker(String symbol) {
    return executeSync(getServiceInterface(), service.getBookTicker(Objects.requireNonNull(symbol)))
        .getSingle();
  }
  ...
  public FeeStatus getFeeStatus() {
    return executeSync(
        getServiceInterface(),
        service.getFeeStatus(
            ExchangeConstants.DEFAULT_RECEIVING_WINDOW, System.currentTimeMillis()));
  }
