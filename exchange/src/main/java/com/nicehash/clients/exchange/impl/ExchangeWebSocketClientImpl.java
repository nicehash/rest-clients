...
public class ExchangeWebSocketClientImpl implements ExchangeWebSocketClient, Closeable {
  ...
  public Closeable onOrderBookEvent(
      String symbol, Levels levels, ClientCallback<OrderBook> callback) {
  ...
  public Closeable onCandlestickEvent(
      String symbol, CandlestickInterval interval, ClientCallback<CandlestickEvent> callback) {
    final String channel = String.format("%s@kline_%s", symbol, interval.getIntervalId());
    return createNewWebSocket(
        channel, new ExchangeWebSocketListener<>(callback, CandlestickEvent.class));
  }
  ...
  public Closeable onTradeEvent(String symbol, ClientCallback<OrderTradeEvent> callback) {
    ...
    return createNewWebSocket(
        channel, new ExchangeWebSocketListener<>(callback, OrderTradeEvent.class));
  }

  public Closeable onMarketTickerEvent(
      String symbol, ClientCallback<AllMarketTickersEvent> callback) {
    final String channel = String.format("%s@ticker", symbol);
    return createNewWebSocket(
        channel, new ExchangeWebSocketListener<>(callback, AllMarketTickersEvent.class));
  }

  public Closeable onUserDataUpdateEvent(
      String listenKey, ClientCallback<UserDataUpdateEvent> callback) {
    return createNewWebSocket(
        listenKey, new ExchangeWebSocketListener<>(callback, UserDataUpdateEvent.class));
  }
