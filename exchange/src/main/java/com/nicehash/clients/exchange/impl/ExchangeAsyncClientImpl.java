 ...
 */
public class ExchangeAsyncClientImpl
    implements ExchangeAsyncClient, ServiceHandle<ExchangeService> {
  ...
  public void getTrades(
      String symbol, Integer limit, ClientCallback<List<TradeHistoryItem>> callback) {
  ...
  public void getHistoricalTrades(
      ...
      Integer limit,
      Long tradeNumber,
      ClientCallback<List<TradeHistoryItem>> callback) {
    executeAsync(
        getServiceInterface(),
        service.getHistoricalTrades(symbol, limit, tradeNumber),
        wrapList(callback));
  }
  ...
  public void getAggTrades(
      ...
      Long startTime,
      Long endTime,
      ClientCallback<List<AggTrade>> callback) {
    executeAsync(
        getServiceInterface(),
  ...
  public void getCandlestickBars(
      ...
      Long startTime,
      Long endTime,
      ClientCallback<List<Candlestick>> callback) {
    executeAsync(
        getServiceInterface(),
        service.getCandlestickBars(symbol, interval.getIntervalId(), limit, startTime, endTime),
        wrapList(callback));
  }

  @Override
  public void getCandlestickBars(
      String symbol, CandlestickInterval interval, ClientCallback<List<Candlestick>> callback) {
  ...
  public void get24HrPriceStatistics(String symbol, ClientCallback<TickerStatistic> callback) {
    executeAsync(
        getServiceInterface(), service.get24HrPriceStatistics(symbol), wrapSingle(callback));
  }
  ...
  public void getOrderStatus(
      OrderStatusRequest orderStatusRequest, ClientCallback<Order> callback) {
    executeAsync(
        getServiceInterface(),
  ...
  public void getOpenOrders(OrderRequest orderRequest, ClientCallback<List<Order>> callback) {
    executeAsync(
        ...
        service.getOpenOrders(
            orderRequest.getSymbol(), orderRequest.getSortDirection(),
  ...
  public void getAllOrders(AllOrdersRequest orderRequest, ClientCallback<List<Order>> callback) {
    executeAsync(
        ...
        service.getAllOrders(
            ...
            orderRequest.getRelationalOp(),
            orderRequest.getSubmitNumber(),
            orderRequest.getSubmitTime(),
            orderRequest.getTerminated(),
            orderRequest.getLimit(),
            orderRequest.getSortDirection(),
  ...
  public void getOrderTrades(
      String symbol, UUID orderId, String clientOrderId, ClientCallback<List<Trade>> callback) {
    executeAsync(
        getServiceInterface(),
        service.getOrderTrades(
            symbol,
            orderId,
            clientOrderId,
            ExchangeConstants.DEFAULT_RECEIVING_WINDOW,
  ...
  public void getAccount(ClientCallback<Account> callback) {
    ...
    executeAsync(
        getServiceInterface(),
        service.getAccount(ExchangeConstants.DEFAULT_RECEIVING_WINDOW, timestamp),
        callback);
  }
  ...
  public void getFeeStatus(ClientCallback<FeeStatus> callback) {
    ...
    executeAsync(
        getServiceInterface(),
        service.getFeeStatus(ExchangeConstants.DEFAULT_RECEIVING_WINDOW, timestamp),
        callback);
  }
  ...
  public void getMyTrades(
      String symbol,
      OrderRelOp relationalOp,
      Long tradeNumber,
      Integer limit,
      SortDirection sortDirection,
      Long recvWindow,
      Long timestamp,
      ClientCallback<List<Trade>> callback) {
    executeAsync(
        getServiceInterface(),
        service.getMyTrades(
            symbol, relationalOp, tradeNumber, limit, sortDirection, recvWindow, timestamp),
        wrapList(callback));
  }
