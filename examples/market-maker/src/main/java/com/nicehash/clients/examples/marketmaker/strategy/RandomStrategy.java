package com.nicehash.clients.examples.marketmaker.strategy;

import com.nicehash.clients.examples.marketmaker.Config;
import com.nicehash.clients.examples.marketmaker.Main;
import com.nicehash.clients.examples.marketmaker.event.Event;
...
public class RandomStrategy implements Strategy {
   ...
   * @param queue Queue with incoming events the strategy uses as input for independent trading
   *     actions
  ...
  public RandomStrategy(
      Config config, Queue<Event> queue, ExchangeClient client, BigDecimal targetPrice) {
    this.config = config;
  ...
  private void processEvent(Event event) {
    try {
      ...
      Main.error("Error processing event " + event.type(), e);
    }
  }
  ...
  private String formatTime(long time) {
    LocalDateTime datetime =
        LocalDateTime.ofEpochSecond(
            (long) (time / 1000d), (int) (time % 1000) * 1000000, ZoneOffset.UTC);
  ...
  private void performAction() {
    ...
    NewOrder newOrder =
        new NewOrder(
            ...
            config.getType() != null
                ? config.getType()
                : config.isNoTake() ? noTakeOrderSide(randomPrice) : randomOrderSide(),
            OrderType.LIMIT,
            TimeInForce.GTC,
            randomQuantity(),
            randomPrice);
    ...
    try {
      ...
      if (newOrder.getSide() == OrderSide.BUY) {
        ...
        if (money.compareTo(reserve) < 0) {
          throw new RuntimeException(
              "Reached money limit - required funds: "
        ...
        if (gold.compareTo(reserve) < 0) {
          throw new RuntimeException(
              "Reached gold limit - required amount: "
                  + reserve.toPlainString()
                  + ", available: "
                  + gold.toPlainString());
        } else {
      ...
      for (Map.Entry<UUID, Order> ent : orders.entrySet()) {
        ...
        if (newOrder.getSide() == oldOrder.getSide()) {
          ...
          System.out.println(
              "Cancelled some order: " + oldOrder.getOrderId() + " [" + oldOrder + "]");
  ...
  private double getBlockLowPrice(
      int block, BigDecimal lowPrice, BigDecimal highPrice, double blockSize) {
