...
public class AuthenticationInterceptor extends AbstractAuthenticationInterceptor {
  ...
  public Response intercept(Chain chain) throws IOException {
    ...
    boolean isApiKeyRequired =
        original.header(HeaderConstants.ENDPOINT_SECURITY_TYPE_APIKEY) != null;
    boolean isSignatureRequired =
        original.header(HeaderConstants.ENDPOINT_SECURITY_TYPE_SIGNED) != null;
    newRequestBuilder
        .removeHeader(HeaderConstants.ENDPOINT_SECURITY_TYPE_SIGNED)
        .removeHeader(HeaderConstants.ENDPOINT_SECURITY_TYPE_SIGNED);
        ...
        if (body != null && body.contentType() != null) {
          ...
          if (bodyStr.length() > 0) {
            newRequestBuilder.method(
                method, RequestBody.create(body.contentType(), bodyStr.getBytes("utf-8")));
          }
        }
      }
